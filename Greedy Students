#include<bits/stdc++.h>
using namespace std;
const int N=125,V=N*N*N;
typedef long long ll;
struct P{int x,y;}a[N];
inline void in(P&a){scanf("%d%d",&a.x,&a.y);}
inline P operator-(const P&a,const P&b){return (P){a.x-b.x,a.y-b.y};}
inline ll cross(const P&a,const P&b){return 1ll*a.x*b.y-1ll*b.x*a.y;}
P X;
inline int getqua(const P&a){
...
inline bool cmp1(const P&a,const P&b){
	int oa=getqua(a-X),ob=getqua(b-X);
	return oa==ob?cross(a-X,b-X)>0:oa<ob;
}
inline bool cmp2(const P&a,const P&b){return a.x==b.x?a.y<b.y:a.x<b.x;}
int n,K,i,j,k,l,r,rk[N][N],aa[N];
bool bb[N];
struct node{int l,r,d,v;ll w,tg;}t[V*20];int tcnt;
